...
inFiles = ["roots04","roots08", "roots12", "roots16", "seeds05","seeds09", "seeds13", "seeds17","weight06","weight10", "weight14", "weight18", "disp07","disp11", "disp15", "disp19"]
...
for plotNum in range(len(inFiles)):
    ...
    title = ["Harvest Maximum","Harvest Minimum", "Plant Maximum", "Plant Minimum"]
    title_pos = [20,20,45,45]
    ...
    if plotNum//cols == rows - 1:
        axes.set_xticks([35,350],["50k","150k"], rotation = 0)
    else:
    ...
    if plotNum%cols == 0: # first column titles
        axes.text(-50, inTitle_pos[plotNum//cols], inTitle[plotNum//cols], fontsize=10, rotation=90)
    if plotNum == cols:
        axes.text(-100, 30, "Gene Value", fontsize=10, rotation=90)
