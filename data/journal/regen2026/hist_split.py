...
for plotNum in range(len(inFiles)):
    ...
    if plotNum % cols == 0:  # First plot in each row
        numEntries = df.sum(axis=0)
        first_plot_col_sums = numEntries  # Store the column sums
    else:
        numEntries = first_plot_col_sums  # Use the stored sums
    ...
    title = ["Total Population","Wild Variant", "Domesticated Variant"]
    title_pos = [80,110,50]
    ...
    if plotNum == cols:
        axes.text(-55, 30, "Gene Value", fontsize=10, rotation=90)
...
f = plt.figure(num=1)
# optional sys.argv[2] = output pdf path (e.g. "split.pdf"); default keeps the batch name imgs/split<NN>.pdf
out = sys.argv[2] if len(sys.argv) > 2 else ("./imgs/split" + sys.argv[1] + ".pdf")
