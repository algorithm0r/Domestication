...
for plotNum in range(len(inFiles)):
    ...
    title = ["Planting Min Root Depth","Planting Min Fecundity", "Planting Max Abscission"]
    title_pos = [15,30,20]
    ...
    if plotNum in [0, 4, 11]:   # Change this list to the subplots you want to highlight
        for _, spine in ax.spines.items():
...
f = plt.figure(num=1)
plt.savefig("./imgs/wildVariants.png")
plt.savefig("./imgs/wildVariants.pdf")
